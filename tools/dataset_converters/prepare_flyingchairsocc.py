...
def parse_args():
    parser = argparse.ArgumentParser(
        description='FlyingChairsOcc dataset preparation')
    parser.add_argument(
        '--data-root',
        type=str,
...
def main():
    args = parse_args()
    split = np.ones(DATASET_SIZE)
    split[VALIDATE_INDICES] = 2
    ...
    occ_dir = osp.join(args.data_root, 'data')

    # data in FlyingChairsOcc dataset has specific suffix
    img1_suffix = '_img1.png'
    img2_suffix = '_img2.png'
    flow_fw_suffix = '_flow.flo'
    flow_bw_suffix = '_flow_b.flo'
    ...
    for i, flag in enumerate(split):
        ...
        data_info = dict(
            ...
            flow_fw_path=osp.join(flow_dir, flow_fw_filenames[i]),
            flow_bw_path=osp.join(flow_dir, flow_bw_filenames[i]),
            occ_fw_path=osp.join(occ_dir, occ_fw_filenames[i]),
            occ_bw_path=osp.join(occ_dir, occ_bw_filenames[i]))
